...
FOCUS: dict[str, dict[str, Any]] = {
    ...
    "ZN": {"route": "zn", "contradiction": "短期伦锌挤仓(低库存+深Back+高注销)托盘,中长期矿紧锭松错配(TC负值vs国内社库高)未解,内盘现货跟不动。基调:挤仓未结束前不空,发令枪=Back走平/库存回升/出口交仓。", "marginal_focus": "伦锌0-3 Back|LME库存/注销|沪伦比|国内社库", "metrics": [
        ("FU00082129", "LME 锌投资基金净多", "手", "Mysteel", "挤仓资金集中度"),
    ...
    "SN": {"route": "sn", "contradiction": "「供应紧」与「需求弱」并存,但目前供应更弱:矿端(缅甸未放量+TC低位)叙事仍在但边际钝化,现货已转松(隐形库存回升、升水回落);需求端结构分化大于总量,9-10月(开工率+库存方向)是关键验证期。当前驱动不在产业边际——下有40万补库支撑、上缺新驱动,锡已金融化为SPX的Beta。基调:高位宽幅震荡,区间思路,回调做多优于追涨。", "marginal_focus": "缅甸月进口(回万吨=证伪)|社库/隐形库存斜率|40万补库线|焊料开工率", "metrics": [
        ...
        ("FU00082093", "LME 锡投资基金净多", "手", "Mysteel", "投机集中度/挤仓风险")]},
    "LC": {"route": "li", "contradiction": "供需已从「过剩叙事」转向「再平衡验证」:周产自6月26.3k降至23.2k吨,SMM总库存10周去库27%至7.24万吨,GFEX仓单自4.9万手降至2.8万手——收紧是产业事实而非单纯政策驱动;但去库斜率8月放缓、仓单小幅回升,价格逼近15.5万后分歧加大。基调:去库持续性定方向,15.5万一线反复争夺,库存拐点证伪前按强势震荡对待。", "marginal_focus": "去库斜率|排产是否止跌|155,000争夺|仓单回升风险", "metrics": [
...
CHART_PICK = {
    "CU": "ID01244863", "AL": "ID00188307", "PB": "a10134441", "ZN": "a10097491",
...
SURVEY_NOTES: dict[str, str] = {
    "SN": "华南(8/14):下游库存压至10-15天刚需采购,40万以下才有补货意愿;消费电子量平+单耗趋势下滑,IGBT/光伏订单翻倍。周报(8/15):隐形库存+3,600吨、升水回落,现货转松。小作文(8/20):沪锡425,550(+1.2%)、锡板块股票篮子+3.28%;矿端三重扰动(银漫停产调查中/佤邦慢复产/刚果金埃博拉)叙事仍在但边际钝化;宏观回暖(美元弱/实际利率下)利多,40万补库线为底,锡仍金融化为SPX Beta——区间对待,回调做多优于追涨。",
    "AL": "华南(8/14):升水强系到货少非需求好,铝棒接单9万吨→半月不足2万吨,9-10月库存显形,俄铝进口有增量。印尼(8月):投产约束在电力非政策,2027增量下修至135万吨,镍铁让电若谈成则上修。",
    "CU": "华南(8/14):废铜开票问题持续半年为历年最长,带票货供不应求、计价系数97→99;国产废铜被阳极板厂抢光,精废收窄倒逼精铜替代,构成实质支撑。小作文(8/20):宏观回暖(美元弱/实际利率下)利多,铜板块股票篮子+1.84%;矿紧(TC深负)长期底+LME Cash-3M挤仓结构仍是主驱动。",
    ...
    "SI": "小作文(8/16):某大厂东部外购电优惠电价取消,拟停全部外购电炉20+台、对部分铝厂封盘停报;但西部或全复产对冲——净减量待验证,现货情绪先紧。小作文(8/20):新疆大厂减产(8月下旬停炉32台/预计减6.5万吨)+宏观回暖,工业硅8,785(+1.6%)午后反弹;但多晶硅反内卷预期降温压制,现货成交有限,净减量待验证。",
    "PS": "小作文(8/16):TH追买1-2000吨硅料(传期货部门做多配合),是仅有的追涨下游;其余下游借美12.4关税前涨价清库存(最晚10月出口、或转口换产地证)不囤货,11月后看淡。小作文(8/20):反内卷倡议后政策预期降温(8/18保供稳价表态),多晶硅37,650回落、盘面贴水现货;8月排产11.1-11.5万吨大增、厂库29万吨高位——增产累库未破,涨价看实质减产执行。",
    ...
    "AU": "小作文(8/20):昨夜FOMC纪要9-3维持偏鹰但市场按鸽定价——财政部长债回购单次上限20→40亿美元压长端,30Y从2007年最高5.337%回落至5.20%,实际利率下行+美元走弱(DXY-0.8%至98.85)=黄金完整利多,现货金破4,522(+4.4%)、沪金973(+2.6%)、贵金属板块股票篮子+6.3%;9月加息概率降至35%。基调:央行购金+实际利率双利多,回调偏多。",
}
